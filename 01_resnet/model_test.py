...
# 导入自定义的ResNet18模型
from model import ResNet18, ResNetBlock
...
def test_data_process():
    ...
    test_data = FashionMNIST(root=r'01_resnet\data',
                              train=False,
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('./01_resnet/models/best_model.pth', weights_only=True))

    # 获取测试数据加载器
    test_dataloader = test_data_process()
    test_model_process(model, test_dataloader) # 调用测试函数
    ...
    # 手动取第 index 个样本
    index = 16   # 样本编号
    dataset_iter = iter(test_dataloader)
    for _ in range(index):
        next(dataset_iter)

    b_x, b_y = next(dataset_iter)
    b_x = b_x.to(device)
    b_y = b_y.to(device)
    ...
    with torch.no_grad():
        ...
        print("预测结果：", classes[result])
        print("真实标签：", classes[label])

    # 注释掉的代码是循环输出所有样本的预测和真实标签，会产生大量输出
    # # 关闭梯度计算
    # with torch.no_grad():
    #     # 遍历测试数据加载器中的每个样本
